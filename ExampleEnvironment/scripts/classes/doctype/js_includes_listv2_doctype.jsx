...
var GlideList2 = Class.create(GwtObservable, {
    ...
    _fireListReady: function () {
        if (!this.isRelated || this.announceOnReady) {
            this._sendAriaLiveMessage(this._generateLiveMessage());
        }
        this.announceOnReady = true;
    ...
    _generateLiveMessage: function () {
        var gm = new GwtMessage();
        var firstRow = this.firstRow.toLocaleString();
        var lastRow = this.lastRow.toLocaleString();
        var grandTotalRows = this.grandTotalRows.toLocaleString();
        if (this.isRelated) {
            if (this.lastRow > 0) {
                return !!this.filter
                    ? gm.getMessage("Filtered {0} related list showing {1} to {2} of {3} records",
                        this.getTitle(), firstRow, lastRow, grandTotalRows)
                    : gm.getMessage("Unfiltered {0} related list showing {1} to {2} of {3} records",
                        this.getTitle(), firstRow, lastRow, grandTotalRows);
            } else {
                return !!this.filter
                    ? gm.getMessage("Filtered {0} related list showing {1} records", this.getTitle(), lastRow)
                    : gm.getMessage("Unfiltered {0} related list showing {1} records", this.getTitle(), lastRow);
            }
        } else {
            if (this.lastRow > 0) {
                return !!this.filter
                    ? gm.getMessage("Filtered {0} list showing {1} to {2} of {3} records",
                        this.getTitle(), firstRow, lastRow, grandTotalRows)
                    : gm.getMessage("Unfiltered {0} list showing {1} to {2} of {3} records",
                        this.getTitle(), firstRow, lastRow, grandTotalRows);
            } else {
                return !!this.filter
                    ? gm.getMessage("Filtered {0} list showing {1} records", this.getTitle(), lastRow)
                    : gm.getMessage("Unfiltered {0} list showing {1} records", this.getTitle(), lastRow);
            }
        }
    },
    _sendAriaLiveMessage: function (message) {
        var delay = this.announceOnReady ? 0 : 5000;
        NOW.accessibility.ariaLivePolite(message, delay);
    },
...
var GlideWidgetVCR = Class.create(GlideListWidget, {
    ...
    _setRowNumbers: function (list) {
        ...
        if (firstRow)
            firstRow.value = list.firstRow.toLocaleString();
        if (descriptionOfSkipTo) {
            getMessage("Showing rows {0} to {1} of {2}", function (msg) {
                descriptionOfSkipTo.innerHTML = new GwtMessage().format(msg, list.firstRow.toLocaleString(), list.lastRow.toLocaleString(), list.totalRows.toLocaleString());
            });
        }
    },
    ...
    $.widget('ui.newtagit', {
        ...
        removeTag: function (tag, animate) {
            ...
            this._trigger('afterTagRemoved', null, {
                tag: tag, tagLabel: this.tagLabel(tag), table: this.options.table,
                rowId: this.options.rowId
            });
        },
        removeTagById: function (tag, id, animate) {
            ...
            this._trigger('afterTagRemoved', null, {
                tag: id, tagLabel: this.tagLabel(tag), table: this.options.table,
                rowId: this.options.rowId
            });
            $("#tags_menu").focus();
        },
