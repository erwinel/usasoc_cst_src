...
GlideFieldPolicy.prototype = {
    ...
    runActions: function (result) {
        ...
        if (result) {
            ...
            if (this.fp.hasOwnProperty("list_actions")) {
                for (var i = 0; i < this.fp.list_actions.length; i++) {
                    var list_action = this.fp.list_actions[i];
                    this._runListAction(list_action, result);
                }
            }
    ...
    _runAction: function (action, result) {
        ...
        if (cleared == 'true' && result) {
            ...
            this._debugAction(field, "cleared", result);
        }
        if (mandatory == 'true') {
    ...
    _runListAction: function (action, result) {
        var list = action.name;
    ...
    if (g_ui_policy && g_ui_policy.length)
        for (var i = 0; i < g_ui_policy.length; i++) {
            ...
            if (uiPolicies[i].scriptRun != null) {
                CustomEvent.fire('page_timing', { name: 'UIOL', child: { description: uiPolicies[i].description, type: uiPolicies[i].scriptRun, sys_id: uiPolicies[i].sys_id, source_table: uiPolicies[i].source_table }, startTime: __ptmr, win: window });
            }
        }
