...
addAfterPageLoadedEvent(function () {
    ...
    document.body.on('click', '[data-type="user_roles_unlock"]', function (evt, element) {
        ...
        modal.on('bodyrendered', function () { gel(ref + 'select_0').focus(); });
...
var GlideBox = Class.create({
    ...
    _resizeIframeShim: function () {
        ...
        var box = this.getBoxElement();
        this._iframeShim.setStyle({
            width: box.getWidth() + 'px', height: box.getHeight() + 'px',
...
function getTableReference(tableName, parentTable, isTemplate, applyTemplateAcls) {
    if (typeof applyTemplateAcls === 'undefined' && isTemplate)
        applyTemplateAcls = true;
    if (firstTable == '')
        firstTable = tableName;
    return Table.get(tableName, parentTable, isTemplate, null, null, applyTemplateAcls);
}
...
function loadFilterTableReference(mft, isTemplate, applyTemplateAcls) {
    ...
    var tableDef = getTableReference(tablepart, null, isTemplate, applyTemplateAcls);
...
function buildFieldsPerType(tableName, tr, descriptorName, fOper, fValue, includeExtended, tableNameFull, filter) {
    ...
    var tableDef = getTableReference(tableName, null, isTemplate, applyTemplateAcls);
    if (!tableDef)
        return;
...
var PersonalizeForm = Class.create({
    ...
    toString: function () { return 'PersonalizeForm'; }
});
$j(function () {
    "use strict";
